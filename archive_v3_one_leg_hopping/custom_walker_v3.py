...
class CustomWalkerWrapper(gym.Wrapper):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # [Reward Engineering V3 - Fix Gait Symmetry]
        # 캥거루 점프(Hopping) 방지 및 교차 보행(Alternating Gait) 유도
        #
        # Formula: R_total = R_forward + R_gait - R_penalties
        
        # 1. Forward Reward (핵심 목표): 전진해야만 큰 점수 획득
        forward_velocity = reduced_obs[2]
        forward_reward = forward_velocity * 5.0
        
        # 2. Gait Reward (교차 보행): 양쪽 다리가 서로 반대 방향으로 움직여야 함
        # reduced_obs 인덱스 매핑 (14차원으로 확장됨):
        # 0~3: Hull
        # 4: Hip1 Angle, 5: Hip1 Vel, 6: Knee1 Angle, 7: Knee1 Vel
        # 8: Hip2 Angle, 9: Hip2 Vel, 10: Knee2 Angle, 11: Knee2 Vel
        # 12~13: Lidar
        
        hip1_vel = reduced_obs[5]
        hip2_vel = reduced_obs[9]
        
        # 두 엉덩이 관절 속도의 곱이 음수이면(서로 반대 방향) 보너스
        # 다리 교차를 강력하게 유도하기 위해 가중치를 크게 줌
        gait_reward = 0.0
        if hip1_vel * hip2_vel < -0.01:
            gait_reward = 2.0
            
        # 3. Stability Penalty (안정성)
        hull_angle = reduced_obs[0]
        angular_velocity = reduced_obs[1]
        unstable_penalty = (abs(hull_angle) ** 2) + (abs(angular_velocity) ** 2)
        
        # 4. Energy Penalty (효율성)
        energy_penalty = np.sum(np.abs(continuous_action)) * 0.005
        self.total_energy_used += np.sum(np.abs(continuous_action))
        
        # 5. Standstill Penalty (나태함 방지)
        standstill_penalty = 0.0
        if abs(forward_velocity) < 0.05:
            standstill_penalty = 0.5
            
        # Total Reward
        custom_reward = forward_reward + gait_reward - unstable_penalty - energy_penalty - standstill_penalty
